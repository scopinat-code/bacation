...
const DEFAULT_ACTIVITIES: ActivityPreference[] = [
...
function normalizeSavedState(value: unknown): PlannerState {
  ...
  const activities = saved.activities ?? DEFAULT_ACTIVITIES;
...
const STEPS = ["시작", "기본정보", "부모 일정", "아이 선택", "함께 조정"];
...
export default function PlannerApp() {
  ...
  const updateBlock = (block: ScheduleBlock, action: "lock" | "delete" | "move") => {
    ...
    if (action === "delete" && block.mustHave) return setToast("‘꼭 넣어줘!’ 활동은 뺄 수 없어요. 3단계에서 체크를 해제해 주세요.");
    ...
    if (action === "lock") {
      ...
      result.blocks[block.day] = result.blocks[block.day].filter((item) => item.id !== block.id);
      result.blocks[alternative.day].push({ ...block, day: alternative.day, start: alternative.start, end: alternative.end, reason: "가족이 ‘다른 시간’을 골라 옮긴 활동이에요." });
  ...
  const dropBlockOnDay = (block: ScheduleBlock, targetDay: DayKey, targetPeriod: "morning" | "afternoon") => {
    ...
    result.blocks[block.day] = result.blocks[block.day].filter((item) => item.id !== block.id);
    result.blocks[targetDay].push({ ...block, day: targetDay, period: targetPeriod, start: slot.start, end: slot.end, reason: "가족이 캘린더에서 직접 끌어 옮긴 활동이에요." });
  ...
  const addManualBlock = (day: DayKey, title: string, start: string, end: string): string | null => {
    ...
    result.blocks[day].push({
      ...
      kind: "manual",
      reason: "부모님이 마지막 단계에서 직접 추가한 일정이에요.",
      locked: false,
      mustHave: false,
    });
...
function Header({ step, onStep, hasResult }: { step: number; onStep: (step: number) => void; hasResult: boolean }) {
  return (
...
function Landing({ hasSaved, onStart, onResume }: { hasSaved: boolean; onStart: () => void; onResume: () => void }) {
  return (
    <section className="landing page-enter">
      <div className="hero-copy">
        <span className="eyebrow">초등 3·4학년 가족을 위한 여름방학 계획</span>
        <h1>빈틈까지 우리답게,<br /><mark>방학 한 칸씩</mark> 맞춰봐요.</h1>
        <p>부모님은 꼭 지켜야 할 시간을, 아이는 하고 싶은 일을 골라요. 다투지 않고 5분이면 여유 있는 시간표가 완성돼요.</p>
        <div className="hero-actions">
          <button className="primary-button big" onClick={onStart}>우리 방학 계획 만들기 <span>→</span></button>
          {hasSaved && <button className="secondary-button" onClick={onResume}>저장한 계획 이어서 보기</button>}
        </div>
      ...
      <div className="hero-visual" aria-label="완성된 시간표 예시">
        ...
        <div className="sample-sheet">
          <div className="sample-header"><span>도토리의 여름방학</span><b>느슨하게, 신나게!</b></div>
          <div className="sample-days">
            <SampleDay day="월" items={["📚 책 읽기", "📌 수영", "☁️ 빈 시간"]} />
            <SampleDay day="수" items={["✏️ 수학", "🌳 밖에서 놀기", "☁️ 빈 시간"]} />
            <SampleDay day="토" items={["🚲 자전거", "🎮 게임", "🍉 가족 시간"]} />
          </div>
          <div className="sample-note">“내가 고른 자전거가 두 번이나 있어!”</div>
        </div>
        <div className="sticker sticker-one">5분 완성</div><div className="sticker sticker-two">여유도 계획!</div>
      </div>
      <div className="how-it-works">
        <article><span>1</span><div><b>부모님이</b><p>학원과 가족 일정을 잠가요.</p></div></article>
        <article><span>2</span><div><b>아이가</b><p>하고 싶은 활동을 골라요.</p></div></article>
        <article><span>3</span><div><b>함께</b><p>결과를 옮기고 확정해요.</p></div></article>
      </div>
    </section>
  );
}
...
function Basics({ state, setState, onNext }: { state: PlannerState; setState: React.Dispatch<React.SetStateAction<PlannerState>>; onNext: () => void }) {
  ...
  return (
    <section className="wizard-page page-enter">
      <PageIntro kicker="1 · 우리 방학 알아보기" title="먼저, 방학의 테두리를 그려요." description="아이의 실명이나 학교는 필요 없어요. 계획에 표시할 별명만 알려주세요." />
      <div className="form-card accent-yellow">
        ...
        <div className="field-grid two">
          ...
          <label><span>시간표에 쓸 별명 <small>선택</small></span><input type="text" maxLength={12} placeholder="예: 도토리" value={state.plan.nickname} onChange={(e) => updatePlan({ nickname: e.target.value })} /></label>
          <label><span>학년</span><select value={state.plan.grade} onChange={(e) => updatePlan({ grade: e.target.value as "3" | "4" })}><option value="3">초등학교 3학년</option><option value="4">초등학교 4학년</option></select></label>
        </div>
      </div>
      ...
      <div className="wizard-actions"><span /><button className="primary-button" onClick={onNext}>부모 일정 넣기 →</button></div>
    </section>
  );
}
...
function ParentSchedule({ state, setState, conflicts, onBack, onNext }: { state: PlannerState; setState: React.Dispatch<React.SetStateAction<PlannerState>>; conflicts: string[]; onBack: () => void; onNext: () => void }) {
  ...
  return (
    <section className="wizard-page wide page-enter">
      <PageIntro kicker="2 · 부모님 차례" title="움직일 수 없는 시간만 잠가주세요." description="학원, 식사, 가족 일정처럼 꼭 지켜야 하는 것만 넣어요. 나머지는 아이가 고를 거예요." />
      <div className="split-layout">
        ...
        <div className="list-panel">
          <h2>지금 잠긴 일정 <span>{state.fixedEvents.length}</span></h2>
      ...
      <div className="form-card exception-card">
        <div className="section-heading"><span className="round-icon mint">🧳</span><div><h2>특별한 기간이 있나요?</h2><p>여행이나 체험학습 기간을 한 번만 입력하면 모든 날짜에 반영해요. 선택 사항이에요.</p></div></div>
      ...
      <div className="wizard-actions"><button className="text-button" onClick={onBack}>← 이전</button><button className="primary-button" onClick={onNext}>아이에게 넘겨주기 →</button></div>
    </section>
  );
}
...
function ChildActivities({ state, setState, generationError, onBack, onGenerate }: { state: PlannerState; setState: React.Dispatch<React.SetStateAction<PlannerState>>; generationError: string; onBack: () => void; onGenerate: () => void }) {
  ...
  const addCustom = () => {
    ...
    setState((current) => ({ ...current, activities: [...current.activities, { id: uid("custom"), category: "play", name: customName.trim(), icon: "⭐", frequency: 1, preference: "any", mustHave: false, selected: true, durationMinutes: 60 }] }));
    setCustomName("");
  };
  ...
  return (
    <section className="wizard-page wide child-page page-enter">
      <PageIntro kicker="3 · 이제 아이 차례!" title={`${state.plan.nickname || "친구"}야, 방학에 뭘 하고 싶어?`} description="마음에 드는 카드를 눌러 골라봐. 너무 많이 골라도 괜찮아. 넣지 못한 건 솔직하게 알려줄게." />
      <div className="selection-count"><span>고른 활동 <b>{selectedCount}</b>개</span><p>공부도, 놀이도, 아무것도 안 하는 시간도 모두 중요해요.</p></div>
      ...
      <div className="activity-grid">
        {state.activities.map((activity) => <article key={activity.id} className={`activity-card category-${activity.category} ${activity.selected ? "selected" : ""}`}>
          <button className="activity-main" aria-pressed={activity.selected} onClick={() => update(activity.id, { selected: !activity.selected })}><span className="activity-icon">{activity.icon}</span><b>{activity.name}</b><i>{activity.selected ? "✓ 골랐어!" : "+ 고르기"}</i></button>
          {activity.selected && <div className="activity-options">
            ...
            <label className="must-toggle"><input type="checkbox" checked={activity.mustHave} onChange={(e) => update(activity.id, { mustHave: e.target.checked })} /><span>⭐ 꼭 넣어줘!</span><small>선택한 횟수를 전부 넣어요.</small></label>
          </div>}
        </article>)}
        <article className="activity-card custom-card"><div className="activity-main"><span className="activity-icon">💭</span><b>내가 하고 싶은 것</b><div className="custom-input"><input aria-label="직접 추가할 활동" maxLength={16} placeholder="직접 써 보기" value={customName} onChange={(e) => setCustomName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addCustom()} /><button onClick={addCustom}>추가</button></div></div></article>
      </div>
      <div className="wizard-actions"><button className="text-button" onClick={onBack}>← 부모님 화면</button><button className="primary-button sparkle" disabled={!selectedCount} onClick={onGenerate}>✨ 우리 시간표 만들기</button></div>
    </section>
  );
}
...
function Results({ state, scheduleRef, onBack, onRegenerate, onBlockAction, onBlockDrop, onManualAdd, onDownload, onPrint, onNotify, busy }: { state: PlannerState; scheduleRef: React.RefObject<HTMLDivElement | null>; onBack: () => void; onRegenerate: () => void; onBlockAction: (block: ScheduleBlock, action: "lock" | "delete" | "move") => void; onBlockDrop: (block: ScheduleBlock, day: DayKey, period: DropPeriod) => void; onManualAdd: (day: DayKey, title: string, start: string, end: string) => string | null; onDownload: () => void; onPrint: () => void; onNotify: (message: string) => void; busy: boolean }) {
  ...
  return (
    <section className="results-page page-enter">
      <div className="result-top no-print"><div><span className="eyebrow">4 · 함께 보고 맞추기</span><h1>짜잔! 우리 가족의 방학 리듬이에요.</h1><p>Google Calendar처럼 활동 블록 전체를 잡고 원하는 요일의 오전·오후에 놓아보세요.</p></div><div className="export-actions"><button className="secondary-button" onClick={onPrint}>🖨️ 바로 인쇄</button><button className="secondary-button" disabled={busy} onClick={onDownload}>{busy ? "이미지 만드는 중..." : "↓ 주간 PNG"}</button></div></div>
      ...
      {viewMode === "weekly" ? <div className="schedule-export" ref={scheduleRef}>
        <div className="print-title"><div><span>방학한칸</span><h2>{state.plan.nickname ? `${state.plan.nickname}의` : "우리 가족"} 여름방학 시간표</h2><p>{state.plan.startDate.replaceAll("-", ".")} — {state.plan.endDate.replaceAll("-", ".")} · 초등 {state.plan.grade}학년</p></div><div className="print-motto">느슨하게,<br />신나게!</div></div>
        ...
        <div className="result-bottom">
          <article className="free-note"><span>☁️</span><div><b>빈 시간도 계획이에요</b><p>심심하면 새로운 놀이가 떠오를 수 있어요. 하루를 꼭 가득 채우지 않아도 괜찮아요.</p></div></article>
          <article className="summary-note"><b>이번 주 한눈에</b><span>하고 싶은 활동 <strong>{totalActivities}</strong>칸</span><span>특별한 기간 <strong>{state.exceptions.length}</strong>개</span></article>
        </div>
        ...
        <p className="print-footnote">부모님이 정한 시간 📌 · 아이가 고른 활동 ⭐ · 방학한칸에서 함께 만들었어요.</p>
      </div> : <VacationCalendar weeks={weeks} nickname={state.plan.nickname} />}
      {manualDay && <ManualScheduleDialog key={manualDay} day={manualDay} onAdd={onManualAdd} onClose={() => setManualDay(null)} />}
      <div className="edit-helper no-print"><div><b>요일과 오전·오후를 직접 정하세요</b><p>블록을 옮기거나 요일 옆의 +를 눌러 부모님 일정을 직접 추가할 수 있어요.</p></div><button className="secondary-button" onClick={regenerate}>↻ 처음부터 다시 배치</button></div>
      ...
      <div className="wizard-actions no-print"><button className="text-button" onClick={onBack}>← 아이 선택 고치기</button><button className="primary-button" onClick={exportPdf}>선택한 일정 PDF 저장</button></div>
    </section>
  );
}
...
function VacationCalendar({ weeks, nickname }: { weeks: VacationWeek[]; nickname: string }) {
  return <div className="vacation-calendar"><div className="vacation-calendar-title"><div><span>방학기간 전체 일정</span><h2>{nickname || "우리 가족"}의 방학을 날짜별로 펼쳤어요.</h2></div><b>{weeks.length}주</b></div>{weeks.map((week, index) => <section key={week.id} className="vacation-week"><header><strong>{index + 1}주차</strong><span>{week.label}</span></header><div className="vacation-days">{week.days.map((day) => <article key={`${day.day}-${day.date ?? "outside"}`} className={`${!day.date ? "outside" : ""} ${day.isException ? "exception" : ""}`}><h3>{day.date ? <><b>{Number(day.date.slice(8))}</b><span>{DAY_LABELS[day.day]}요일</span></> : <span>방학 전·후</span>}</h3><div>{day.blocks.map((block) => <p key={block.id} className={`category-${block.category}`}><small>{block.kind === "fixed" || block.kind === "manual" ? `${block.start}-${block.end}` : block.period === "morning" ? "오전" : "오후"}</small><b>{block.icon} {block.title}</b></p>)}{day.date && !day.blocks.length && <em>☁️ 여유로운 날</em>}</div></article>)}</div></section>)}</div>;
}
